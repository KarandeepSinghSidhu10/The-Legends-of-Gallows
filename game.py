...
def list_roles():
    typewriter("\nChoose your destiny (roles):", 0.03)
    for num, r in ROLES.items():
        typewriter(Fore.CYAN + f"{num}. {r['name']} - {r['desc']}", 0.015)
    ...
    while True:
        try:
            ...
            if choice in ROLES:
                ...
                typewriter(Fore.RED + "Invalid choice. Pick a valid role number.", 0.02)
        except ValueError:
            typewriter(Fore.RED + "Please enter a number.", 0.02)

def select_difficulty():
    typewriter(Fore.MAGENTA + Style.BRIGHT + "\nChoose your challenge level:", 0.03)
    for idx, level in enumerate(DIFFICULTY_LEVELS, start=1):
        typewriter(Fore.YELLOW + f"{idx}. {level}", 0.02)
    ...
    while True:
        try:
            ...
            if 1 <= c <= len(DIFFICULTY_LEVELS):
                ...
                typewriter(Fore.RED + f"Choose between 1 and {len(DIFFICULTY_LEVELS)}.", 0.02)
        except ValueError:
            typewriter(Fore.RED + "Please enter a number.", 0.02)

def select_category_for_role(role_name):
    typewriter(Fore.MAGENTA + Style.BRIGHT + f"Categories for {role_name}:", 0.03)
    for idx, cat in enumerate(SHARED_CATEGORIES, start=1):
        typewriter(Fore.CYAN + f"{idx}. {cat}", 0.02)
    while True:
        try:
            ...
            if 1 <= c <= len(SHARED_CATEGORIES):
                ...
                typewriter(Fore.RED + f"Choose between 1 and {len(SHARED_CATEGORIES)}.", 0.02)
        except ValueError:
            typewriter(Fore.RED + "Please enter a number.", 0.02)
...
def play_game():
    ...
    typewriter(Fore.CYAN + role["desc"], 0.06)

    difficulty = select_difficulty()
    typewriter(Fore.GREEN + f"Challenge set to: {difficulty}", 0.04)

    category = select_category_for_role(role_name)
    
    # Retrieve the word (this will be done again if Ninja uses Shadow Step)
    ...
    # Pirate: Treasure Sense check
    if role.get("treasure_sense") and any(c in secret_word for c in 'esa'):
        lives += 1
        max_lives += 1
        typewriter(Fore.GREEN + "The Pirate's Treasure Sense grants +1 Life!", 0.04)

    # Explorer: Ancient Map check
    if role.get("ancient_map") and len(secret_word) >= 7:
        hint_uses = 3
        typewriter(Fore.GREEN + "The Explorer's Ancient Map grants +1 Hint!", 0.04)
    else:
        hint_uses = 2
    ...
    consecutive_correct = 0 # for Bard
    last_guess_was_miss = False # for Warrior
    detective_miss_count = 0 # for Detective
    vowels = 'aeiou'
    first_vowel_guessed = False # for Wizard
    bard_shield_active = False # for Bard
    
    # Rogue/Ninja: Reveal a Letter at start
    if role.get("reveal_letter"):
        ...
        if available_letters:
            ...
            typewriter(Fore.GREEN + f"As a {role_name}, a secret rune '{reveal}' is revealed!", 0.04)

    typewriter(Fore.MAGENTA + Style.BRIGHT + f"\n🌟 The Quest Begins — {category} ({difficulty}) — {len(secret_word)} runes to uncover 🌟\n", 0.04)
    
    # Ninja: Shadow Step available tracker
    ninja_swap_available = role.get("shadow_step")

    # Rogue: Swift Escape available tracker
    ...
    while lives > 0 and '_' in display_word:
        ...
        if guess == 'swap' and ninja_swap_available:
            ...
            typewriter(Fore.YELLOW + Style.BRIGHT + "Shadow Step activated! The word has been swapped. The quest restarts!", 0.04)
            continue
        elif guess == 'swap' and role.get("shadow_step"):
            typewriter(Fore.RED + "Shadow Step already used!", 0.02)
            continue
            
        # Rogue: Swift Escape
        if guess == 'escape' and rogue_escape_available:
            typewriter(Fore.YELLOW + "Swift Escape requires a failed attempt to activate. Guess a letter!", 0.03)
            continue

        # Mage: Arcane Insight
        if guess == 'vowel' and role.get("arcane_insight") and hint_uses > 0:
            hint_uses -= 1
            typewriter(Fore.GREEN + "Arcane Insight activated.", 0.03)
            letter_type = input(Fore.WHITE + "Enter a letter to check (a-z): ").lower().strip()
            
            if len(letter_type) != 1 or not letter_type.isalpha():
                typewriter(Fore.RED + "Invalid rune. Insight wasted!", 0.02)
            elif letter_type in vowels:
                typewriter(Fore.CYAN + f"The rune '{letter_type}' is a VOWEL!", 0.03)
            else:
                typewriter(Fore.CYAN + f"The rune '{letter_type}' is a CONSONANT!", 0.03)
            continue
        elif guess == 'vowel' and role.get("arcane_insight"):
            typewriter(Fore.RED + "No hints left for Arcane Insight!", 0.02)
            continue
        ...
        if guess == 'hint':
            if hint_uses > 0:
                ...
                typewriter(Fore.GREEN + f"Oracle's Wisdom: {hint}", 0.03)
                hint_uses -= 1
                continue
            else:
                typewriter(Fore.RED + "No hints left! Trust your instincts.", 0.02)
                continue
        
        # General Input Validation
        if len(guess) != 1 or not guess.isalpha():
            typewriter(Fore.RED + "Invalid spell! You must utter a single rune!", 0.02)
            continue
        if guess in guessed_letters:
            typewriter(Fore.RED + "You've already cast that spell!", 0.02)
            continue
        ...
        if is_correct:
            typewriter(Fore.GREEN + f"Success! The rune '{guess}' glows brightly – a piece of the word is revealed!", 0.02)
            
            newly_revealed = reveal_letter_in_word(secret_word, display_word, guess)
            
            # --- Correct Guess Logic ---
            consecutive_correct += 1
            consecutive_incorrect = 0 # Reset forgiving counter
            
            # Warrior: Adept Striker (negates last miss)
            if role.get("adept_striker") and last_guess_was_miss and lives < max_lives:
                lives += 1
                typewriter(Fore.GREEN + "Adept Striker: Your swift hit negated your last miss, restoring 1 life!", 0.02)
            last_guess_was_miss = False

            # Druid: Nature's Aid (vowel life restore)
            if role.get("druid") and guess in vowels and lives < max_lives:
                lives += 1
                typewriter(Fore.GREEN + "Nature heals you! A life is restored.", 0.02)
            
            # Wizard: Elemental Affinity (first vowel reveals all)
            if role.get("elemental_affinity") and guess in vowels and not first_vowel_guessed:
                ...
                typewriter(Fore.GREEN + "Elemental Affinity: The first vowel guess revealed all its runes!", 0.02)

            # Paladin: Divine Smite (free guess on 5th unique letter revealed)
            ...
            if role.get("divine_smite") and unique_revealed_count == 5 and newly_revealed > 0:
                typewriter(Fore.YELLOW + "Divine Smite: You revealed a core rune! Take a free bonus guess (no penalty for a miss)!", 0.03)
                # Next guess will skip standard input loop logic, just display game and continue
                pass # Allow the game loop to continue to the next guess immediately

            # Detective: Motive Analysis (extra letter after 2 misses)
            if role.get("motive_analysis") and detective_miss_count >= 2:
                ...
                if remaining_correct_letters:
                    ...
                    typewriter(Fore.CYAN + f"Motive Analysis: Clues connected! Rune '{extra_reveal}' is also revealed!", 0.03)
                detective_miss_count = 0 # Reset miss count
            
            # Druid: Fungal Growth (extra reveal on G, R, O)
            if role.get("fungal_growth") and guess in 'gro':
                ...
                if remaining_correct_letters:
                    ...
                    reveal_letter_in_word(secret_word, display_word, extra_reveal)
                    guessed_letters.add(extra_reveal)
                    typewriter(Fore.GREEN + f"Fungal Growth: Hidden life found! Rune '{extra_reveal}' is also revealed!", 0.03)
            
            # Bard: Inspiring Performance (shield granted)
            if role.get("inspiring_performance") and consecutive_correct == 3 and not bard_shield_active:
                bard_shield_active = True
                typewriter(Fore.YELLOW + Style.BRIGHT + "Inspiring Performance! You have a temporary shield against the next miss!", 0.04)

        else: # Guess is incorrect
            
            # Bard: Inspiring Performance (shield activated)
            if bard_shield_active:
                typewriter(Fore.YELLOW + "The Sorcerer's spell rebounds off your Inspiring Performance shield! No life lost!", 0.02)
                bard_shield_active = False
                consecutive_correct = 0 # Reset correct streak
                continue
            
            # Rogue: Swift Escape activation
            if guess != 'escape' and rogue_escape_available and input(Fore.MAGENTA + "Activate Swift Escape to negate this miss? (y/n): ").lower().strip() == 'y':
                typewriter(Fore.YELLOW + "Swift Escape! You vanish and avoid the penalty!", 0.03)
            ...
            if role.get("forgiving"):
                ...
                if consecutive_incorrect < 3:
                    typewriter(Fore.YELLOW + f"Misfire, but your divine shield holds! ({3 - consecutive_incorrect} more until it counts.)", 0.02)
                    continue
            
            typewriter(Fore.RED + f"Misfire! The rune '{guess}' fades – the sorcerer draws nearer!", 0.02)
            lives -= 1
            incorrect_guesses += 1
            consecutive_incorrect = 0 # Reset once the miss counts

        if role.get("bard"):
            encouragements = ["Keep the faith!", "You're a legend in the making!", "The runes bend to your will!"]
            typewriter(Fore.GREEN + random.choice(encouragements), 0.02)

    display_game(secret_word, display_word, guessed_letters, lives, incorrect_guesses, role_name)
    if '_' not in display_word:
        typewriter(Fore.GREEN + Style.BRIGHT + f"\n🎉 Victory! As the {role_name}, you have deciphered the word '{secret_word}' and banished the Hangman Sorcerer! {random.choice(WIN_MESSAGES)}", 0.03)
    else:
        typewriter(Fore.RED + Style.BRIGHT + f"\n💀 Defeat! The word was '{secret_word}'. {random.choice(LOSE_MESSAGES)}", 0.03)

def show_main_menu():
    typewriter(Fore.MAGENTA + Style.BRIGHT + "=== THE TAVERN OF CHOICES ===", 0.03)
    typewriter(Fore.CYAN + "What will you do, adventurer?", 0.03)
...
def show_roles_library():
    typewriter(Fore.MAGENTA + Style.BRIGHT + "\n=== ROLE LIBRARY ===\n", 0.03)
    for num, r in ROLES.items():
        typewriter(Fore.CYAN + f"{num}. {r['name']}: {r['desc']}", 0.03)
        sample_role_name = r['name']
        if sample_role_name in ROLE_WORDS:
            for cat in SHARED_CATEGORIES:
                samples = []
                for difficulty in DIFFICULTY_LEVELS:
                    words = ROLE_WORDS[sample_role_name].get(cat, {}).get(difficulty, [])
                    if words:
                        samples.extend(words[:2]) 
                
                if samples:
                    typewriter(Fore.YELLOW + f"  {cat} (Sample Words): " + ", ".join(samples), 0.01)
...
def main():
    while True:
        ...
        if choice == '1':
            ...
            if again != 'y':
                typewriter(Fore.CYAN + "Farewell, adventurer! May your tales inspire bards.", 0.03)
                sys.exit()
        elif choice == '2':
            show_roles_library()
        elif choice == '3':
            typewriter(Fore.CYAN + "Farewell, adventurer! Return when the stars call you.", 0.03)
            sys.exit()
        else:
            typewriter(Fore.RED + "Choose a valid option (1-3).", 0.02)